 ...
 * @type {RegExp}
 */
const imgNameReg = /^\d{8}-[\u4e00-\u9fffa-zA-Z]+$/;
const imgType = /\.(jpe?g|png|bmp)$/i;
const imgSize = 1024 * 1024 * 5;

export const imageRule = (img, rules) => {
  const name = img.name;
  if (typeof rules === 'undefined') {
    rules = [1, 2, 3];
  }
  if (!imgType.test(name) && rules.includes(2)) {
    return {type: 2, msg: '格式不正确，只支持png/jpg/bmp格式', name};
  } else if (!(img.size < imgSize) && rules.includes(3)) {
    return {type: 3, msg: '大小已超过5M', name};
  } else if ( !(imgNameReg.test(name.split('.')[0])) && rules.includes(1)) {
    return {type: 1, msg: '命名规则不正确', name};
  }
  return '';
};
