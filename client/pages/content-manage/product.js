...
class Product extends Component {
  ...
  handleChangeState(id, s, code) {
    return () => {
      ...
      const { pageSize, pageNum, cityCode, name, status, flag, business } = this.state;
      ...
      confirm({
        ...
        onOk() {
          appActions.loading(true).then(() => {
            return contentManageActions.changeProductState({
              id,
              status: changedStatus,
              weight: s === 0 ? productSort.get('0') + 1 : null,
              cityCode: code
            })
            .then(() => {
              return Promise.all([
  ...
  handlePaginationChange(page, pageSize) {
    const { cityCode, name, status, flag, business } = this.state;
    const { appActions, contentManageActions } = this.props;
    appActions.loading(true).then(() => {
      return contentManageActions.getProductList({
        pageNum: page,
        pageSize,
        cityCode,
        name,
  ...
  render() {
    ...
    const productList = contentManage.get('productList') && contentManage.get('productList');
    ...
    const columns = [{
      ...
      render: (flag, data) => {
        if (flag === 1) {
          return (<span style={{width: '35px', height: '18px', color: 'white', border: '1px solid red',
          borderRadius: '10px', background: 'red', textAlign: 'center', display: 'inline-block'}}>热门</span>)
        }
        if (flag === 0) {
          return (<span style={{width: '35px', height: '18px', color: 'white', border: '1px solid #ccc',
          borderRadius: '10px', background: '#ccc', textAlign: 'center', display: 'inline-block'}}>热门</span>)
        }
      }
    }, {
      title: '状态',
      ...
      <Layout className='content-common' id='product-area' >
        ...
        >
          ...
          <Row gutter={80}>
            ...
            <Col span={8}>
              <FormItem {...formItemLayout} label='热门产品'>
