...
class User extends Component {
  ...
  componentWillMount() {
    ...
    appActions.loading(true).then(() => {
      return contentManageActions.getUserList({
        pageNum,
        pageSize
      })
    }).then((data) => {
  ...
  handlePaginationChange(page, pageSize) {
    const { appActions, contentManageActions } = this.props;
    const { phone, title, isReal } = this.state;
    appActions.loading(true).then(() => {
      return contentManageActions.getUserList({
        pageNum: page,
        pageSize,
  ...
  render() {
    ...
    const userList = contentManage.get('userList') && contentManage.get('userList');
      ...
      <Layout className='content-common' id='user-area' >
        ...
        >
          <Row gutter={10}>
            ...
            <Col span={6}>
              <FormItem {...formItemLayout} label='实名状态'>
                { getFieldDecorator( 'isReal', {initialValue: ''})(
